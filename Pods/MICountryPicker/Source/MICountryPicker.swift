...
@objc public protocol MICountryPickerDelegate: class {
    ...
    @objc  func countryPicker(picker: MICountryPicker, didSelectCountryWithName name: String, code: String, dialCode: String, countryImage: UIImage)
}
   ...
   // var searchController: UISearchController!
    ...
    var unsourtedCountries: [MICountry] {
        ...
        for countryCode in countriesCodes {
            let displayName = (Locale.current as NSLocale).displayName(forKey: NSLocale.Key.countryCode, value: countryCode) //locale.dis   // displayNameForKey(NSLocaleCountryCode, value: countryCode)
    ...
    public var showCallingCodes = true
...
//        }
        ...
        cell.imageView!.image = UIImage.init(named: bundle + country.code.lowercased() + ".png", in: Bundle(for: MICountryPicker.self), compatibleWith: nil)
        return cell
    }
...
//        } else {
            country = sections[indexPath.section].countries[indexPath.row]
            let bundle = "assets.bundle/"
            let countryImage = UIImage.init(named: bundle + country.code.lowercased() + ".png", in: Bundle(for: MICountryPicker.self), compatibleWith: nil)
     //   }
       // delegate?.countryPicker(picker: self, didSelectCountryWithName: country.name, code: country.code)
        delegate?.countryPicker(picker: self, didSelectCountryWithName: country.name, code: country.code, dialCode: country.dialCode, countryImage: countryImage!)
